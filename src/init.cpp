 ...
 */
static const char* BITCOIN_PID_FILENAME = "vargameshd.pid";
/**
...
void SetupServerArgs(ArgsManager& argsman, bool can_listen_ipc)
{
    ...
    argsman.AddArg("-port=<port>", strprintf("Listen for connections on <port> (default: %u, testnet3: %u, testnet4: %u, signet: %u, regtest: %u). Not relevant for I2P (see doc/i2p.md). If set to a value x, the default onion listening port will normally be set to x+1. If that port conflicts with the RPC port, x+2 is used.", defaultChainParams->GetDefaultPort(), testnetChainParams->GetDefaultPort(), testnet4ChainParams->GetDefaultPort(), signetChainParams->GetDefaultPort(), regtestChainParams->GetDefaultPort()), ArgsManager::ALLOW_ANY | ArgsManager::NETWORK_ONLY, OptionsCategory::CONNECTION);
...
bool AppInitParameterInteraction(const ArgsManager& args)
{
    ...
    // VargaMesh Testnet v1 is a first-class VargaMesh test network.
    if (chain == ChainType::TESTNET) {
        LogInfo("VargaMesh Testnet v1 selected. Test coins have no value.\n");
    }
...
bool AppInitMain(NodeContext& node, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if (args.IsArgSet("-datadir") && !args.GetPathArg("-datadir").is_absolute()) {
        LogWarning("Relative datadir option '%s' specified, which will be interpreted relative to the "
                   "current working directory '%s'. This is fragile, because if vargameshd is started in the future "
                   "from a different location, it will be unable to locate the current data files. There could "
                   "also be data loss if vargameshd is started while in a temporary directory.",
    ...
    uint16_t default_bind_port_onion = default_bind_port + 1;
    const uint16_t rpc_port = static_cast<uint16_t>(
        args.GetIntArg("-rpcport", BaseParams().RPCPort()));
    if (default_bind_port_onion == rpc_port) {
        default_bind_port_onion = default_bind_port + 2;
    }

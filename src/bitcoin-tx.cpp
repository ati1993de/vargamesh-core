...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.GetBoolArg("-version", false)) {
        ...
        std::string strUsage = CLIENT_NAME " vargamesh-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.GetBoolArg("-version", false)) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "The vargamesh-tx tool is used for creating and modifying VargaMesh transactions.\n\n"
                "vargamesh-tx can be used with \"<hex-tx> [commands]\" to update a hex-encoded VargaMesh transaction, or with \"-create [commands]\" to create a hex-encoded VargaMesh transaction.\n"
                "\n"
                "Usage: vargamesh-tx [options] <hex-tx> [commands]\n"
                "or:    vargamesh-tx [options] -create [commands]\n"
                "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded VargaMesh transaction

...
const char * const BITCOIN_CONF_FILENAME = "vargamesh.conf";
...
{
    // Windows:
    //   old: C:\Users\Username\AppData\Roaming\VargaMesh
    //   new: C:\Users\Username\AppData\Local\VargaMesh
    // macOS: ~/Library/Application Support/VargaMesh
    // Unix-like: ~/.vargamesh
#ifdef WIN32
    // Windows
    // Check for existence of datadir in old location and keep it there
    fs::path legacy_path = GetSpecialFolderPath(CSIDL_APPDATA) / "VargaMesh";
    if (fs::exists(legacy_path)) return legacy_path;

    // Otherwise, fresh installs can start in the new, "proper" location
    return GetSpecialFolderPath(CSIDL_LOCAL_APPDATA) / "VargaMesh";
#else
    fs::path pathRet;
...
#ifdef __APPLE__
    // macOS
    return pathRet / "Library/Application Support/VargaMesh";
#else
    // Unix-like
    return pathRet / ".vargamesh";
#endif
#endif
}

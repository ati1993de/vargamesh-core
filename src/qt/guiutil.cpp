...
static std::string DummyAddress(const CChainParams &params)
{
    ...
    case ChainType::MAIN:
        addr = "vm1p35yvjel7srp783ztf8v6jdra7dhfzk5jaun8xz2qp6ws7z80n4tqs65mfe";
        break;
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a VargaMesh address (e.g. %1)").arg(
...
{
    // return if URI is not valid or is no vargamesh: URI
    if(!uri.isValid() || uri.scheme() != QString("vargamesh"))
        return false;
...
QString formatBitcoinURI(const SendCoinsRecipient &info)
{
    ...
    QString ret = QString("vargamesh:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;
